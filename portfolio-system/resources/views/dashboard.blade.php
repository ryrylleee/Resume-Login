<x-authed-layout>
    {{-- This title uses the $resume variable --}}
    <x-slot name="title">{{ $resume->name }}'s Dashboard</x-slot>

    {{-- Date Formatting Helper (Needed here since this view uses it) --}}
    @php
        /**
         * Converts a YYYY-MM string to Month YYYY format.
         */
        function formatResumeDate($isoDate) {
            ...
            try {
                // Assuming Carbon is available in your environment (Laravel standard)
                return \Carbon\Carbon::createFromFormat('Y-m', $isoDate)->format('F Y');
            } catch (\Exception $e) {
                // Fallback for bad data (shows the raw value)
                return $isoDate;
            }
        }
    @endphp

    {{-- This layout is based on your public.resume.blade.php file --}}
    <div class="resume-layout">
        <div class="resume-left">
            <div class="profile-img-wrap">
                {{-- 🎯 FIX 1: Display Profile Picture from Path --}}
                @if($resume->profile_picture_path)
                    <img src="{{ asset('storage/' . $resume->profile_picture_path) }}" 
                         alt="Profile Picture" 
                         class="profile-pic">
                @else
                    {{-- Placeholder image if no picture is set --}}
                    <img src="https://placehold.co/140x140/6a89cc/FFF?text=Profile" 
                         alt="Profile Picture" 
                         class="profile-pic">
                @endif
            </div>
            ...
            <div class="resume-skills">
                <h4>Skills</h4>  @foreach ($resume->skills_list as $skill)
                ...
                <div class="education-section">
                    ...
                    @foreach ($resume->education as $edu)
                        <div class="edu-block">
                            ...
                            {{-- 🎯 FIX 2: Format Education Dates --}}
                            <div>
                                {{ formatResumeDate($edu['start_date'] ?? null) }} - {{ formatResumeDate($edu['end_date'] ?? null) }}
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="organizations-section">
                    ...
                    @foreach ($resume->organizations as $org)
                        <div class="org-block">
                            ...
                            {{-- 🎯 FIX 2: Format Organization Dates --}}
                            <div>
                                {{ formatResumeDate($org['start_date'] ?? null) }} - {{ formatResumeDate($org['end_date'] ?? null) }}
                            </div>
                        </div>
                    @endforeach
                </div>
